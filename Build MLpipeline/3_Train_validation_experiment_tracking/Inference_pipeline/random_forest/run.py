...
def get_inference_pipeline(args):
    ...
    nlp_transformer = make_pipeline(SimpleImputer(strategy="constant",fill_value=""),
                                    reshape_to_1d,
                                    TfidfVectorizer(binary=True)) #Term Frequency (TF) in a document * Inverse Document Frequency (IDF)<how unique a term is across all documents>, then normalize this quantity
